...
def makeperiodictable():
    ...
    def addelement(Z, row, column):
        ...
        ax.add_patch(rect)
        ax.annotate(pp.find_element(Z), (cx, cy), color='k', weight='bold', fontsize=11, ha='center', va='center')
        ax.annotate(str(Z), (cx-0.25, cy-0.25), color='k', fontsize=8, ha='center', va='center')
        elements[Z] = {
            "rect" : rect,
            "value": 0.0
        }
